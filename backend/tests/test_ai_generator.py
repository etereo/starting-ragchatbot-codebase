...
class TestAIGenerator:
    ...
    def test_init_openai(self, mock_config):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            mock_openai_class.assert_called_once_with(api_key=mock_config.OPENAI_API_KEY)
    ...
    def test_init_gemini(self, mock_config):
        ...
        with patch('google.generativeai') as mock_genai:
            ...
            mock_genai.configure.assert_called_once_with(api_key=mock_config.GEMINI_API_KEY)
    ...
    def test_generate_response_anthropic_without_tools(self, mock_config, prevent_actual_api_calls):
        ...
        prevent_actual_api_calls['anthropic'].return_value.messages.create.return_value = mock_response
        ...
        call_args = prevent_actual_api_calls['anthropic'].return_value.messages.create.call_args
        assert call_args[1]["messages"][0]["content"] == "Test query"
        assert "tools" not in call_args[1]
    
    def test_generate_response_anthropic_with_tools_no_tool_use(self, mock_config, mock_tool_manager, prevent_actual_api_calls):
    ...
    def test_generate_response_anthropic_with_single_round_tool_use(self, mock_config, mock_tool_manager, prevent_actual_api_calls):
        ...
        final_response.content[0].text = "Based on the search results, here's the answer..."
        final_response.stop_reason = "end_turn"
        
        # Configure mock to return different responses on subsequent calls
        prevent_actual_api_calls['anthropic'].return_value.messages.create.side_effect = [
            tool_use_response,
            final_response
        ]
        
        # Configure tool manager
        mock_tool_manager.execute_tool.return_value = "Search results: machine learning content"
        ...
        assert prevent_actual_api_calls['anthropic'].return_value.messages.create.call_count == 2
    
    def test_generate_response_anthropic_tool_execution_error(self, mock_config, mock_tool_manager, prevent_actual_api_calls):
    ...
    def test_generate_response_openai(self, mock_config, prevent_actual_api_calls):
        ...
        prevent_actual_api_calls['openai'].return_value.chat.completions.create.return_value = mock_completion
        ...
        call_args = prevent_actual_api_calls['openai'].return_value.chat.completions.create.call_args
    ...
    def test_generate_response_gemini_exception_fallback(self, mock_config, prevent_actual_api_calls):
        ...
        type(mock_response).text = property(lambda self: (_ for _ in ()).throw(Exception("Text access error")))
    ...
    def test_generate_response_with_conversation_history(self, mock_config, prevent_actual_api_calls):
    ...
    def test_handle_tool_execution_multiple_tools(self, mock_config, mock_tool_manager, prevent_actual_api_calls):
        ...
        mock_tool_manager.execute_tool.assert_any_call("get_course_outline", course_title="ML Course")
    
    def test_generate_response_anthropic_two_round_sequential_tool_use(self, mock_config, mock_tool_manager, prevent_actual_api_calls):
        ...
        final_response = Mock()
        final_response.content = [Mock()]
        final_response.content[0].text = "Based on both the outline and search, here's a comprehensive answer..."
        final_response.stop_reason = "end_turn"
        ...
        assert result == "Based on both the outline and search, here's a comprehensive answer..."
        
        # Verify both tools were executed in sequence
        assert mock_tool_manager.execute_tool.call_count == 2
        mock_tool_manager.execute_tool.assert_any_call("get_course_outline", course_title="Machine Learning Basics")
        mock_tool_manager.execute_tool.assert_any_call("search_course_content", query="neural networks", course_name="Deep Learning")
        
        # Verify three API calls were made (round 1 + round 2 + final)
        assert prevent_actual_api_calls['anthropic'].return_value.messages.create.call_count == 3
    
    def test_generate_response_anthropic_max_rounds_reached(self, mock_config, mock_tool_manager, prevent_actual_api_calls):
        ...
        final_response = Mock()
        final_response.content = [Mock()]
        final_response.content[0].text = "Here's my final answer based on the search results..."
        final_response.stop_reason = "end_turn"
        ...
        final_call_args = prevent_actual_api_calls['anthropic'].return_value.messages.create.call_args_list[2]
    ...
    def test_anthropic_api_error_handling(self, mock_config, prevent_actual_api_calls):
        ...
        prevent_actual_api_calls['anthropic'].return_value.messages.create.side_effect = Exception("API Error")
...
class TestAIGeneratorToolIntegration:
    ...
    def test_end_to_end_tool_flow(self, mock_config, prevent_actual_api_calls):
        ...
        final_response = Mock()
        final_response.content = [Mock()]
        final_response.content[0].text = "Machine learning is a powerful technique for data analysis."
