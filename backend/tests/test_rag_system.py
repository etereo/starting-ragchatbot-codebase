...
class TestRAGSystemDocumentProcessing:
    ...
    @patch("rag_system.AIGenerator")
    @patch("rag_system.SessionManager")
    def test_add_course_document_processing_error(
        self,
        mock_session_mgr,
        mock_ai_gen,
        mock_vector_store,
    ...
    @patch("rag_system.SessionManager")
    @patch("os.path.exists")
    def test_add_course_folder_not_found(
        self,
        mock_exists,
        mock_session_mgr,
        mock_ai_gen,
...
class TestRAGSystemQuery:
    ...
    @patch("rag_system.AIGenerator")
    @patch("rag_system.SessionManager")
    def test_query_anthropic_successful(
        self,
        mock_session_mgr,
        mock_ai_gen,
        mock_vector_store,
        ...
        mock_tool_mgr.get_last_links.return_value = [
            ("ML Course", "https://example.com/ml")
        ]
    ...
    @patch("rag_system.AIGenerator")
    @patch("rag_system.SessionManager")
    def test_query_openai_with_manual_context(
        self,
        mock_session_mgr,
        mock_ai_gen,
        mock_vector_store,
        ...
        mock_generator = mock_ai_gen.return_value
        mock_generator.generate_response.return_value = (
            "OpenAI response based on context"
        )

        # Setup tool manager mock
        mock_tool_mgr = Mock()
        mock_tool_mgr.execute_tool.return_value = (
        ...
        mock_tool_mgr.execute_tool.assert_called_once_with(
            "search_course_content", query="Explain machine learning"
        )
    ...
    @patch("rag_system.AIGenerator")
    @patch("rag_system.SessionManager")
    def test_query_with_session_history(
        self,
        mock_session_mgr,
        mock_ai_gen,
        mock_vector_store,
        ...
        mock_session.add_exchange.assert_called_once_with(
            "session123", "Follow up question", response
        )
    ...
    @patch("rag_system.AIGenerator")
    @patch("rag_system.SessionManager")
    def test_query_source_links_in_response(
        self,
        mock_session_mgr,
        mock_ai_gen,
        mock_vector_store,
        ...
        mock_tool_mgr.get_last_links.return_value = [
            ("Test Course", "https://example.com/course")
        ]
    ...
    def test_complete_query_flow_with_mocks(self, mock_config):
        ...
        with (
            ...
            mock_tool_mgr.get_last_sources.return_value = [
                "<a href='https://example.com'>ML Course - Lesson 1</a>"
            ]
            mock_tool_mgr.get_last_links.return_value = [
                ("ML Course - Lesson 1", "https://example.com")
            ]
            rag.tool_manager = mock_tool_mgr

            # Execute query
            response, sources = rag.query(
                "What is machine learning?", session_id="test_session"
            )

            # Verify complete flow
            assert "Machine learning is a field" in response
            assert (
                "[Open source: ML Course - Lesson 1](https://example.com)" in response
            )
...
class TestRAGSystemRealDocumentScenarios:
    ...
    def test_folder_processing_with_mixed_files(self, mock_config, tmp_path):
        ...
        (tmp_path / "course2.pdf").write_text(
